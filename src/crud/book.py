...
class CrudBook:
    ...
    def get_company(self, id_book: int):
        readers_reading = self.session.query(models.User.id.label('id_user'),
                                   models.User.nickname,
                                   models.User.photo,
                                   models.Status.status.label('status')
                            )\
                            ...
                            .join(models.Status, models.Status.id == models.UserBook.fk_status)\
                            .limit(16).all()
        readers_read = self.session.query(models.User.id.label('id_user'),
                                   models.User.nickname,
                                   models.User.photo,
                            )\
                            ...
                            .join(models.UserBook, models.UserBook.fk_user == models.User.id)\
                            .limit(16).all()
        readers_to_read = self.session.query(models.User.id.label('id_user'),
                                   models.User.nickname,
                                   models.User.photo,
                            )\
        ...
        return {
            ...
            'readers_to_read': {
                ...
                'list': readers_to_read,
            }
        }

    def get_company_status(self, id_book: int, id_status: int):
            query = self.session.query(models.User.id.label('id_user'),
                                   models.User.nickname,
                                   models.User.photo,
                            )\
